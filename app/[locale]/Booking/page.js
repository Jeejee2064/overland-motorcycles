...
import {
  Calendar, User, MessageSquare, Bike, MapPin, Check,
  AlertCircle, X, MessageCircle, ChevronRight, ChevronLeft,
  Edit2, Receipt, CreditCard
} from 'lucide-react';
...
import { createBooking, checkBikesAvailableByModel } from '@/lib/supabase/bookings';
...
const MODELS = {
  Himalayan: {
    id: 'Himalayan',
    label: 'Royal Enfield Himalayan',
    shortLabel: 'Himalayan 450',
    cc: '452cc',
    maxBikes: 6,
    pricing: HIMALAYAN_PRICING,
    // Replace with your actual image paths
    image: '/himalayan.jpg',
    specs: ['452cc single-cylinder', 'Adventure touring', 'Up to 6 available'],
  },
  CFMoto700: {
    id: 'CFMoto700',
    label: 'CF Moto 700 CL-X',
    shortLabel: 'CF Moto 700',
    cc: '693cc',
    maxBikes: 1,
    pricing: CFMOTO_PRICING,
    image: '/CFMoto.jpeg',
    specs: ['693cc parallel-twin', 'Sport adventure', '1 available'],
  },
};
...
const BookingPage = () => {
  ...
  // Steps: 1 = model, 2 = dates+qty, 3 = personal info, 4 = review+pay
  const [currentStep, setCurrentStep]       = useState(1);
  const [selectedModel, setSelectedModel]   = useState(null); // 'Himalayan' | 'CFMoto700'
  ...
  const maxBikes           = currentModelConfig?.maxBikes ?? 1;
  ...
  const handleDateRangeChange = async (range) => {
    ...
    if (start && end) {
      ...
      try {
        ...
        if (available < needed) {
          setAvailabilityError(`Sorry, only ${available} ${currentModelConfig?.shortLabel ?? 'motorcycle'}(s) available for the selected dates.`);
        } else {
  ...
  const calculatePrice = () => {
    ...
    if (days === 0) return 0;
    const pricing = currentPricing.find(p => p.days === days);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const available = await checkBikesAvailableByModel(startDate, endDate, selectedModel);
      const needed    = numBikes;
      if (available < needed) {
        showModal('error', `Sorry, only ${available} ${currentModelConfig?.shortLabel}(s) available for the selected dates. Please adjust your selection or choose different dates.`);
      ...
      showModal('error', error.message || 'There was an error processing your booking. Please try again or contact us.');
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black text-white">
      ...
      {modal.isOpen && (
        <Modal isOpen={modal.isOpen} onClose={closeModal} type={modal.type} message={modal.message} onConfirm={modal.onConfirm} />
      )}
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="step1" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }}>
                <div className="text-center mb-6">
                  ...
                  <p className="text-gray-400 text-sm md:text-base max-w-2xl mx-auto">
                    Select the model you'd like to rent. Both are available for multi-day adventures across Panama.
                  </p>
                </div>
                  ...
                  {Object.values(MODELS).map((model) => (
                    <motion.button
                      ...
                      onClick={() => {
                        ...
                        // Reset qty to 1 when switching models
                        setFormData(prev => ({ ...prev, bikeQuantity: '1' }));
                        setAvailabilityError('');
                      }}
                    ...
                    >
                      ...
                      <div className={`p-4 ${selectedModel === model.id ? 'bg-yellow-400/10' : 'bg-gray-800/50'}`}>
                        ...
                        <ul className="space-y-1">
                          {model.specs.map((spec, i) => (
                            <li key={i} className="text-xs text-gray-400 flex items-center gap-1.5">
                              <Check size={10} className="text-yellow-400 flex-shrink-0" />
                              {spec}
                            </li>
                          ))}
                        </ul>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-700 flex items-baseline gap-1">
                          ...
                          <span className="text-xl font-black text-yellow-400">${model.pricing[0].price}</span>
            ...
            {currentStep === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }}>
                ...
                {/* Quantity selector — hidden for CF Moto (auto = 1) */}
                {selectedModel === 'Himalayan' && (
                  <div className="mb-4">
                    <p className="text-center text-sm text-gray-400 mb-3">How many bikes?</p>
                    <div className="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-3">
                      {[1, 2, 3, 4, 5, 6].map((num) => (
                        ...
                        >
                          ...
                          <div className="text-xs text-gray-400">{num === 1 ? 'Bike' : 'Bikes'}</div>
                          {formData.bikeQuantity === num.toString() && (
                            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-lg font-bold text-white mt-1">
                          ...
                          {selectedModel === 'Himalayan' && ` × ${formData.bikeQuantity} ${formData.bikeQuantity === '1' ? 'Bike' : 'Bikes'}`}
                        </div>
                      </div>
            ...
            {currentStep === 3 && (
              <motion.div key="step3" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }}>
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-6">
                  <div>
                    <label className="block text-xs font-semibold text-gray-300 mb-1">{t('firstName')} *</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-300 mb-1">{t('lastName')} *</label>
            ...
            {/* ── STEP 4: Review & Pay — unchanged structure, model info added ── */}
            {currentStep === 4 && (
