...
export async function POST(request) {
  try {
    ...
    // mode: 'balance' = remaining 50%, 'full' = total price
    ...
    const amount = isFull
      ...
      : (parseFloat(booking.total_price) - parseFloat(booking.down_payment)).toFixed(2);

    const desc = isFull
      ? `Full Payment - ${booking.first_name} ${booking.last_name}`
      : `Remaining Balance - ${booking.first_name} ${booking.last_name}`;
    ...
    const customFields = [
      ...
      { id: 'paymentType', nameOrLabel: 'Payment Type', value: isFull ? 'FULL' : 'BALANCE' },
    ];
