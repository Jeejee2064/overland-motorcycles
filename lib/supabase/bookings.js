...
export const updateBookingPayment = async (bookingId, paid) => {
...
export async function checkBikesAvailable(startDate, endDate) {
  try {
    ...
    console.log('Supabase RPC request:', {
      p_start_date: formatDate(startDate),
      p_end_date: formatDate(endDate)
    });
 ...
 */
export async function checkBikesAvailableByModel(startDate, endDate, model) {
  try {
 ...
 */
export async function checkBikesAvailabilityRangeByModel(model) {
  try {
    if (!model) return {};
...
export async function createBooking(bookingData, additionalRiders = []) {
  try {
    ...
    const { data: booking, error: bookingError } = await supabase
      ...
      .insert([{
        ...
        motorcycle_model: bookingData.motorcycle_model || 'Himalayan',
        ...
        auth_status:      bookingData.auth_status || null,
        balance_status:   bookingData.balance_status || null,
        paid:             false,
      }])
      .select()
      .single();
    ...
    if (additionalRiders.length > 0) {
      const { error: ridersError } = await supabase
        .from('booking_riders')
        .insert(additionalRiders.map((r, i) => ({
      ...
      if (ridersError) console.error('Error inserting additional riders:', ridersError);
    }
...
export async function getMotorcycleCalendarWithPhone(startDate = null, endDate = null) {
  ...
  const [{ data: bookingsData }, { data: ridersData }, { data: motoAssignments }] = await Promise.all([
    supabase.from('bookings').select('id, phone, email, first_name, last_name').in('id', bookingIds),
