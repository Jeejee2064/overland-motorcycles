...
function badge(paid, paidLabel = 'PAID ✓', pendingLabel = 'Pending') {
  return paid
    ? `<span style="background:#DEF7EC;color:#03543F;padding:4px 10px;border-radius:4px;font-weight:600;">${paidLabel}</span>`
    : `<span style="background:#FEF3C7;color:#92400E;padding:4px 10px;border-radius:4px;font-weight:600;">${pendingLabel}</span>`;
}

export function generateManualBookingEmailHTML(booking, motorcycles, modelLabel) {
  const downPaymentPaid  = booking.payment_status === 'paid';
  const depositAuthed    = booking.auth_status === 'authorized';
  const balancePaid      = booking.balance_status === 'captured' || booking.paid === true;
  const remainingPayment = downPaymentPaid
    ? booking.total_price - booking.down_payment
    : booking.total_price;
        ...
        <div class="content">
          ...
          <div class="section">
            ...
            <div class="detail-row">
              ...
              <span class="detail-value">${badge(downPaymentPaid, `$${booking.down_payment.toFixed(2)} PAID ✓`, `$${booking.down_payment.toFixed(2)} — Pending`)}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">Remaining Balance:</span>
              <span class="detail-value">${badge(balancePaid, `$${(booking.total_price - booking.down_payment).toFixed(2)} PAID ✓`, `$${remainingPayment.toFixed(2)} — Due at pickup`)}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">Security Deposit:</span>
              <span class="detail-value">${badge(depositAuthed, `$${booking.deposit.toFixed(2)} Authorized`, `$${booking.deposit.toFixed(2)} — Due at pickup`)}</span>
            </div>
          </div>
          ...
          <div class="warning">
            ...
            <ul>
              ...
              ${!balancePaid ? `<li>Bring remaining balance: <strong>$${remainingPayment.toFixed(2)}</strong></li>` : ''}
              ${!depositAuthed ? `<li>Security deposit: <strong>$${booking.deposit.toFixed(2)}</strong> (refundable, due at pickup)</li>` : ''}
